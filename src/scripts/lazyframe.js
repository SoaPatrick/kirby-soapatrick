const Lazyframe = () => {
  ...
  function loop(el) {
    if (
      ...
      el.classList.contains("lazyframe--loaded")
    )
      return;
  ...
  function setup(el) {
    const attr = Array.prototype.slice
      .apply(el.attributes)
      .filter((att) => att.value !== "")
      .reduce((obj, curr) => {
        let name =
          curr.name.indexOf("data-") === 0
            ? curr.name.split("data-")[1]
            : curr.name;
        obj[name] = curr.value;
        return obj;
      }, {});

    const options = Object.assign({}, settings, attr, {
      y: el.offsetTop,
  ...
  function scroll() {
    ...
    elements.filter((el) => el.settings.y < height).forEach(initElement);
    ...
    const onScroll = debounce(() => {
      ...
      if (up) {
        elements
          .filter(
            (el) =>
              el.settings.y < height + lastY &&
              el.settings.initialized === false
          )
          .forEach(initElement);
      }
